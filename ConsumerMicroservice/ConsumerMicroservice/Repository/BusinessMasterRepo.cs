        ...
        public List<BusinessMaster> GetBusinessMaster()
        {
            using (StreamReader sr = new StreamReader("Data.BusinessMaster.csv"))
            {
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    Bmaster.Add(new BusinessMaster() { Consumer = new Consumer() { ID = Convert.ToInt32(values[0]), Name = values[1], DOB = Convert.ToDateTime(values[2]), Email = values[3], PAN = values[4], Business = new Business() { ID = Convert.ToInt32(values[5]), BusinessType = values[6], CapitalInvested = Convert.ToDecimal(values[7]), AnnualTurnover = Convert.ToDecimal(values[8]), TotalEmployees = Convert.ToInt32(values[9]) } }, BusinessValue = Convert.ToInt32(values[10]) });
                }
            }
